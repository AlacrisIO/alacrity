...
const decodeGameCreationEvent_ = (data, blockNumber, txHash) => {
...
// blockNumber and txHash will probably always be there,
// contract iff the game creates its own contract, although even if not,
// there will presumably still be some kind of game ID within the mother Ethereum contract.
const gameBlockData_ = game => {
    const {contract, blockNumber, txHash} = game;
    return {contract, blockNumber, txHash}}
// This can be generated by the contract from the place of user addresses in its events.
const isGameRelevantToUser_ = (game, userAddress) =>
    game.player0 == userAddress || optionalAddressMatches(game.player1, userAddress)
const isGameInitiator_ = (game, userAddress) => game.player0 == userAddress;

gameBlockData = gameBlockData_
isGameRelevantToUser = isGameRelevantToUser_
isGameInitiator = isGameInitiator_
...
// Used by the frontend for now.
// Will also be used the backend when we recompute state from events instead of using queryState
// TODO: use event tracking, not queryState
const GameResult = Object.freeze({TheyWin: 0, Draw: 1, YouWin: 2});
const gameResult = (yourHand, theirHand) => (yourHand + 4 - theirHand) % 3;
...
const createNewGame = (wagerInWei, escrowInWei, player1, hand0) => {
